...
/**
 * A class representing a Fibonacci heap, obtained via https://www.keithschwarz.com/interesting/code/?dir=fibonacci-heap
 *
...
public final class FibonacciHeap<T> {
     ...
     * private.
     */
    public static final class EntryHeap<T> {
        private int     mDegree = 0;       // Number of children
        private boolean mIsMarked = false; // Whether this node is marked

        private EntryHeap<T> mNext;   // Next and previous elements in the list
        private EntryHeap<T> mPrev;

        private EntryHeap<T> mParent; // Parent in the tree, if any.

        private EntryHeap<T> mChild;  // Child node, if any.
         ...
         */
        EntryHeap(T elem, double priority) {
            mNext = mPrev = this;
            mElem = elem;
            mPriority = priority;
        }
     ...
     */
    public EntryHeap<T> enqueue(T value, double priority) {
        checkPriority(priority);

        /* Create the entry object, which is a circularly-linked list of length
         * one.
         */
        EntryHeap<T> result = new EntryHeap<T>(value, priority);
    ...
    public void enqueue(EntryHeap<T> result) {
    	 /* Merge this singleton list with the tree list. */
        mMin = mergeLists(mMin, result);
    ...
    public EntryHeap<T> dequeueMin() {
         ...
         */
        List<EntryHeap<T>> treeTable = new ArrayList<EntryHeap<T>>();
         ...
         */
        List<EntryHeap<T>> toVisit = new ArrayList<EntryHeap<T>>();
         ...
         * of that list.
         */
        for (EntryHeap<T> curr = mMin; toVisit.isEmpty() || toVisit.get(0) != curr; curr = curr.mNext)
            toVisit.add(curr);

        /* Traverse this list and perform the appropriate unioning steps. */
        for (EntryHeap<T> curr: toVisit) {
            ...
            while (true) {
                ...
                EntryHeap<T> other = treeTable.get(curr.mDegree);
                 ...
                 */
                EntryHeap<T> min = (other.mPriority < curr.mPriority)? other : curr;
                EntryHeap<T> max = (other.mPriority < curr.mPriority)? curr  : other;
     ...
     */
    public void decreaseKey(EntryHeap<T> entry, double newPriority) {
     ...
     */
    public void delete(EntryHeap<T> entry) {
     ...
     */
    private static <T> EntryHeap<T> mergeLists(EntryHeap<T> one, EntryHeap<T> two) {
        ...
        if (one == null && two == null) { // Both null, resulting list is null.
             ...
             *
             */
            EntryHeap<T> oneNext = one.mNext; // Cache this since we're about to overwrite it.
            one.mNext = two.mNext;
            one.mNext.mPrev = one;
            two.mNext = oneNext;
            two.mNext.mPrev = two;
     ...
     */
    private void decreaseKeyUnchecked(EntryHeap<T> entry, double priority) {
     ...
     */
    private void cutNode(EntryHeap<T> entry) {
